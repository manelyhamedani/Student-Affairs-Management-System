...
int reserve(const char *self_id, const char *date, const char *meal, const char *food_id, int ratio, int is_daily) {
    ...
    sprintf(sql, "insert into RESERVED_MEAL (reserved_meal_id, student_id, self_id, food_id, type, date, daily_reserved) values(%d, '%s', %s, %s, '%s', '%s', %d, 0);", reserved_meal_id, current_user.username, self_id, food_id, meal, date, is_daily);
    ...
    sprintf(sql, "update STUDENTS set balance = balance - (%d * (select price from FOOD where food_id = %s)) where student_id = '%s';", ratio, food_id, current_user.username);
    rc = sqlite3_exec(db, sql, NULL, NULL, &errmsg);
    if (rc != SQLITE_OK) {
        fprintf(stderr, "SQL error: %s\n", errmsg);
        sqlite3_free(errmsg);
...
int cancel_reserve(const char *date, const char *meal) {
    ...
    sprintf(sql, "update STUDENTS set balance = balance + 0.9 * (select price from FOOD where food_id = %d) where student_id = '%s';", food_id, current_user.username);
    rc = sqlite3_exec(db, sql, NULL, NULL, &errmsg);
    if (rc != SQLITE_OK) {
        fprintf(stderr, "SQL error: %s\n", errmsg);
        sqlite3_free(errmsg);
...
int daily_reserve(const char *self_id, const char *food_id) {
    ...
    int rc = sqlite3_exec(db, sql, callback, &lunch_time_end, &errmsg);
    if (rc != SQLITE_OK) {
        fprintf(stderr, "SQL error: %s\n", errmsg);
        sqlite3_free(errmsg);
        return permission_denied;
    ...
    char condition[max_size];
    set_date_time();
    sprintf(condition, "where self_id = %s and food_id = %s and date = '%s' and count > 0 and %d - %s >= 30 ", self_id, food_id, current_date_time.date, lunch_time_end, current_date_time.time);
    if (is_exists("MEAL_PLAN", condition) == 1) {
        return reserve(self_id, current_date_time.date, "lunch", food_id, 2, 1);
    }
    sprintf(condition, "where self_id = %s and food_id = %s and date = '%s' and count > 0 and %d - %s >= 30 ", self_id, food_id, current_date_time.date, dinner_time_end, current_date_time.time);
    if (is_exists("MEAL_PLAN", condition) == 1) {
...
int define_agent(const char *date, const char *meal, const char *user_id) {
    ...
    sprintf(condition, "where student_id = '%s' and date = '%s' and type = '%s' ", current_user.username, date, meal);
    if (is_exists("RESERVED_MEAL", condition) != 1) {
        return not_found;
    }
    char *errmsg = NULL;
...
int check_news(void) {
    ...
    int exist = not_found;
    set_date_time();
    sprintf(sql, "select * from NEWS where julianday(end_date) - julianday('%s') >= 0;", current_date_time.date);
    int rc = sqlite3_exec(db, sql, print_news, &exist, &errmsg);
    if (rc != SQLITE_OK) {
        fprintf(stderr, "SQL error: %s\n", errmsg);
        sqlite3_free(errmsg);
        return permission_denied;
